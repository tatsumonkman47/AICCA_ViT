...
def train_model(training_checkpoint_path,**kwargs):
    logger = CSVLogger("test_logs", name="lighting_log_test")
    trainer = pl.Trainer(default_root_dir=os.path.join(CHECKPOINT_PATH, {MODEL_NAME}),
                         accelerator="gpu" if str(device).startswith("cuda") else "cpu",
                         devices=1,
                         max_epochs=180,
                         logger=logger,
                         callbacks=[ModelCheckpoint(save_weights_only=True, verbose=True, save_last=True, mode="max", monitor="val_acc"),
...
MODEL_NAME = "ViT_wide_1e-4lr"
########################################################################################################
########################################################################################################
training_checkpoint_path = f"/home/tdmonkman/scratch-midway2/AICCA_proj/{MODEL_NAME}/lightning_logs/version_4/checkpoints/last.ckpt"
